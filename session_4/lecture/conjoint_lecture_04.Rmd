...
1. Used in most applications of conjoint survey experiments
...
1. Simple Mean difference
    1. Calculate the average rating (or probability to be chosen) of all female candidates
    2. Calculate the average rating (or probability to be chosen) of all male candidates 
...
3. Votes candidate would obtain in a head to head competition
...
3. Sum over all possible opponents
...
Let's now calculate use the normalization constant to calculate the AMCE obtained from the sum over all possible opponents 
...
3. Q: What are the units of observation in CJ analysis? 
...
6. In order to correctly estimate the Standard Error we need to take into account the clustered nature of the data 
...
1. **Sandwich estimators** (also called robust variance estimator)
    ...
    3. Sandwich: Take into account the variance heterogeneity
2. Choice Models 
2. Multilevel models 
    1. Level 1: CJ tasks 
